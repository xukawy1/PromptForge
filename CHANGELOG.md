...
## Phase 15.3（v0.8.2）
- 加速：Ollama keep_alive 模型常驻显存 + 共享 HTTP 连接池 + 流式进度生成。
- 采集结果窗体新增进度条与已用时长显示。
- 未配置模型时一键跳转模型中心（全应用覆盖）。
- 工作台新增「一键释放内存」。


## Phase 15.4（v0.8.3）
- 修复右键复制粘贴菜单黑屏不可用（QMenu/QMenuBar 配色）。
- 采集结果窗体改左右布局；提示词拆分改全手动触发。
- 拆分规则强化：按序号/标题/图片内容逐条分类识别。


## Phase 15.5（v0.8.4）
- 结果窗体左栏拆分：原文预览（上）+ 综合提示词（下）双框。
- 拆分修复：多策略 JSON 解析 + 长文分段分析 + 图片 OCR 并入分类。
- 图片缩略图与点击放大预览。
- 打开窗体后台预热模型，加快首次分析。


## Phase 15.6（v0.8.5）
- 拆分引擎强化：八仙级 few-shot、多线程分段、失败重试与整段兜底、图片逐张成条，确保必出清单。
- 模型中心：API 配置多套保存/切换/删除（删除并清空密钥）、默认行高亮、后台自动同步。
- 图片反推：缩略图列表 + 点击大图预览后反推。


## Phase 15.7（v0.8.6）
- 模型中心 API 区改为供应商管理模式：列表 + 添加弹窗 + 设为当前 + 测试刷新。
- 删除供应商时连同后台模型记录、密钥与失效默认一并清除，防止 API 被他人套用。


## Phase 15.8（v0.8.7）
- 图片反推新增「删除选中图片」：移除待解析图片（软件内副本一并删除，原始文件不受影响）。


## Phase 16.1（v0.8.8）
- 历史记录新增删除功能：单条删除 + 清空全部（均有确认提醒；进行中任务受保护）。


## Phase 16.2（v0.8.9）
- Skill 工坊进入卡顿修复：素材改轻量索引懒加载 + 分类名一次映射 + 正文按需读取。
- skill 生成空结果修复：系统提示词节选（优先 SKILL.md）+ 空结果重试 + 规则拼接兜底。


## Phase 16.3（v0.8.10）
- Skill 工坊：已安装 Skill 支持重命名（含重名校验与启动防重复安装）。
- Skill 工坊：素材支持手动输入，优先于列表选择，可直接按 Skill 规范扩写。


## Phase 16.4（v0.8.11）
- Skill 工坊：提示词素材选择与手动输入素材各新增「一键清空」按钮，避免扩写内容混淆。


## Phase 16.5（v0.8.12）
- Skill 工坊：已安装 Skill 的详细内容与说明支持编辑。
